#ifndef _HASH_
#define _HASH_
#define ToM11(x) 2.0f * x - 1.0f                //将 [0, 1] 范围的值映射到 [-1, 1] 范围 例：float signedNoise = ToM11(noise);        // 转为 [-1,1]，用于正负方向扰动

//hashAB：生成对应维度的伪随机数
//A：输入维度（1 = float, 2 = float2, 3 =float3）
//B：输出维度（1 = float, 2 = float2, 3 = float3）
