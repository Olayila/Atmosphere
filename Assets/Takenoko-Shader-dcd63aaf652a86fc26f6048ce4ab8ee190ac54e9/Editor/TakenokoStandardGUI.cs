...
public class TakenokoStandardGUI : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            if (MenuFoldout(material, "RenderMode"))
            {
            ...
            if (MenuFoldout(material, "Main Paramater"))
            {
                GUILayout.Space(5);
                EditorGUI.indentLevel++;
            ...
            if (MenuFoldout(material, "Emission"))
            {
                using (new EditorGUILayout.VerticalScope("HelpBox"))
                {
                    GUILayout.Space(5);
            ...
            if (MenuFoldout(material, "Thin-Film"))
            {
                using (new EditorGUILayout.VerticalScope("HelpBox"))
                {
                    GUILayout.Space(5);
            ...
            if (MenuFoldout(material, "Lightmap"))
            {
                GUILayout.Space(5);
                using (new EditorGUILayout.VerticalScope("HelpBox"))
                {
            ...
            if (MenuFoldout(material, "Experimental"))
            {
                GUILayout.Space(5);
                EditorGUI.indentLevel++;
                using (new EditorGUILayout.VerticalScope("HelpBox"))
    ...
    public void setMaterialProperty(MaterialProperty[] properties)
    {
        ...
        ThinFilmMenu = FindProperty("_ClothMenu", properties);
    ...
    bool MenuFoldout(Material material, string menuPropName,  string tooltip = "")
    {
        ...
        //return state;
        // 从材质读取展开状态
        float foldoutValue = material.GetFloat(menuPropName);
        bool isOpen = foldoutValue > 0.5f;

        // 创建带 Tooltip 的 GUIContent
        GUIContent content = new GUIContent(menuPropName, tooltip);

        // 可选：自定义 Foldout 样式（加粗 + 大字体）
        GUIStyle foldoutStyle = new GUIStyle(EditorStyles.foldout);
        foldoutStyle.fontStyle = FontStyle.Bold;
        foldoutStyle.fontSize = 13;  // 可调整

        // 绘制原生小三角 Foldout
        bool newState = EditorGUILayout.Foldout(isOpen, content, true, foldoutStyle);

        // 如果用户点击了三角，更新材质上的值（记住状态）
        if (newState != isOpen)
        {
            material.SetFloat(menuPropName, newState ? 1f : 0f);
            // 触发重绘，避免延迟
