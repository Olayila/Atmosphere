...
//在给定表面法线 n 的方向上，从 L1 阶球谐（Spherical Harmonics）表示的环境光中计算得到的漫反射辐照度（Diffuse Irradiance）强度。
//使用方式：
//float irradiance = shEvaluateDiffuseL1Geomerics(L0, L1, normalWS);
//float3 diffuseColor = albedo * irradiance; // 乘以材质颜色得到最终漫反射光
...
inline float3 lightMapEvaluate(Texture2D lightmap, float2 uv)
{
    ...
    //先保证运行
    return DecodeLightmap(SAMPLE_TEXTURE2D(lightmap, lightmap_trilinear_clamp_sampler, uv), float4(2.0, 2.0, 0.0, 0.0));
