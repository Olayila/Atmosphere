...
// PBR BRDF 核心（Disney Diffuse + GGX Specular + Fresnel + Energy Conservation）
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/BRDF.hlsl"

// 完整光照（调用 BRDF 函数 + 主光/额外光/环境光融合）
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
#include "../common/color.hlsl"
#include "../common/constant.hlsl"
#include "StandardSampler.hlsl"
#include "StandardMaterial.hlsl"
#include "StandardForwardThinFilm.hlsl"
#include "StandardForwardCloth.hlsl"
...
inline void EvaluateBSDF_TK(
    inout float3 diffuse, inout float3 specular, float3 normalWorld, TK_GIInput giInput, MaterialParameter matParam)
{
    float3 lightDirection = giInput.mainLight.direction;
    float3 viewDirection = giInput.worldViewDir;
    float3 lightEmission = giInput.mainLight.color * giInput.atten;
...
TK_GIInput giInput, MaterialParameter matParam)
{
    ...
    float3 envBRDF = giInput.indirectSpecular * EnvBRDFApprox(F0, matParam.roughness, ndotv);
    specular = envBRDF;
}
...
TK_GIInput giInput, MaterialParameter matParam)
{
    EvaluateSpecularBSDF_TK(specular, normalWorld, giInput.worldViewDir, matParam, giInput.mainLight.direction, giInput.mainLight.color, giInput.atten);
    float3 specularGI = 0.0f;
    EvaluateSpecularGI_TK(specularGI, normalWorld, giInput, matParam);
    specular += specularGI;
}
