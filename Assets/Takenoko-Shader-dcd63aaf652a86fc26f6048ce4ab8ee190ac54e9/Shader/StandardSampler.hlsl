...
/// 随机采样函数，用在mipmap中
/// </summary>
/// <param name="tex">要采样的纹理</param>
/// <param name="uv">原始 UV 坐标</param>
/// <param name="ss">采样器状态（控制过滤模式、地址模式等）</param>
float4 StochSample_float(Texture2D tex, float2 uv, SamplerState ss)
{
    //float2x2(1.0, 0.0, -0.57735027, 1.15470054) 是一个斜切矩阵（shear matrix）,为防止规则采样将采样点把正方形网格扭曲成等边三角形网格
    float2 skewUV = mul(float2x2(1.0, 0.0, -0.57735027, 1.15470054), uv * 3.464);
    float stochasticStrength = 0.01;
    int2 vertID = int2(floor(skewUV));
    //取小数部分，得到在当前三角形单元内的归一化局部坐标（barycentric-like）。
    ...
    float2 dy = ddy(uv);
    //hash 函数，得到三个随机变量
    float2 randomA = uv + frac(sin(fmod(float2(dot(vertA, float2(127.1, 311.7)), dot(vertA, float2(269.5, 183.3))), 3.14159)) * 43758.5453) * stochasticStrength;
    float2 randomB = uv + frac(sin(fmod(float2(dot(vertB, float2(127.1, 311.7)), dot(vertB, float2(269.5, 183.3))), 3.14159)) * 43758.5453) * stochasticStrength;
    float2 randomC = uv + frac(sin(fmod(float2(dot(vertC, float2(127.1, 311.7)), dot(vertC, float2(269.5, 183.3))), 3.14159)) * 43758.5453) * stochasticStrength;
    //tex.SampleGrad(ss, randomA, dx, dy);unity内置，根据当前uv，按照dxdy返回mipmap级别 
...
//强制使用手动计算的显式的梯度（ddx/ddy）来采样 2D 纹理，而不是让 GPU 自动计算梯度。
//默认模式
//随机采样模式：Stochastic Sampling：每帧/每个像素给 UV 加一个微小随机偏移（基于噪声纹理或蓝噪声）。
//这样采样点不会整齐对齐，破坏了规则的干扰图案（Moiré）。
//结果是：闪烁变成轻微的均匀噪声（grain），人眼更不敏感。
...
#define UNPACK_NORMAL_TK(packedNormal, bumpScale)  UnpackNormalScale(packedNormal, bumpScale)
...
{
    //高度自定义的视差贴图（ ParallaxMapping） 采样器
